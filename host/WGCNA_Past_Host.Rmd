...
save file
```{r}
pdf(file = "Past_Host_dendrogram_SampleID.pdf", width = 13, height = 9)
par(cex = 0.6)
par(mar = c(0,4,2,0))
plot(sampleTree, main = "Porites astreoides Host",  sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2) 
...
save plot
```{r}
pdf("Past_Host_dendrogram_Trait-heatmap.pdf") 
...
```{r}
save(dynamicMods_hybrid, MEList, MEs, MEDiss, METree, file= "Past_Host_WGCNA_ME_Network.RData")
```
...
save plot
```{r}
pdf(file="Past_Host_WGCNA_MergeNetwork.pdf", width=20, height=20)
#pdf(file = "___.pdf", wi = 9, he = 6)
...
#MEs_final = mergedMEs
save(mergedMEs, moduleLabels, moduleColors_merged, MEDiss_merged, METree_merged, geneTree, file= "Past_Host_WGCNA_ME_Network_merged.RData")
```
...
```{r}
pdf(file="Past_Host_WGCNA_EigengeneNetwork_Dendro_smaller.pdf", width=10, height=6) # 10
...
```{r}
pdf(file="Past_Host_WGCNA_ModuleOrder_bySample.pdf", width=10, height=12) 
...
save plot
```{r}
pdf("Past_Host_module-trait_heatmap.pdf")
...
write.csv(file = "Past_Host_WGCNA_module_genes.csv", x = summary)
```
