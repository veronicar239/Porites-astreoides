...
```{r}
# save plot
#pdf("Past_Host_PCA_total-exp_group.pdf")
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Lagoon-Ojo vs. Lagoon-Control (762 genes)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
```{r}
# save plot
pdf("Past_Host_PCA_LO_LC_all-indiv.pdf")
...
```{r}
# save plot
#pdf("Past_Host_PCA_LO_LC_subset.pdf")
...
```{r}
# save plot
pdf("Past_Host_PCA_OO_OC_all-indiv.pdf")
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Control vs. Lagoon-Control (456 genes)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
```{r}
# save plot
pdf("Past_Host_PCA_OC_LC_all-indiv.pdf")
...
```{r}
# save plot
#pdf("Past_Host_PCA_OC_LC_subset.pdf")
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Ojo vs. Lagoon-Ojo (284 genes)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
```{r}
# save plot
pdf("Past_Host_PCA_OO_LO_all-indiv.pdf")
...
```{r}
# save plot
#pdf("Past_Host_PCA_OO_LO_subset.pdf")
...
```{r}
permanova_overall.interaction <- adonis(dist_overall ~ Origin_type + Destination_type + Origin_type*Destination_type, data = meta, permutations=9999, na.rm = TRUE)
...
## Pairwise PERMANOVA - Destination
```{r}
pairwise.adonis(dist_overall, factors = meta$Destination_type,  p.adjust.m = 'fdr', perm = 9999)
