...
```{r}
summary2$Destination <- c("Control", "Control", "Ojo", "Ojo", "Ojo vs. Control", "Ojo vs. Control", "Ojo vs. Control", "Ojo vs. Control")
...
plot.combo <- ggplot(summary2, aes(x = group, fill = LFC)) + 
  ...
  theme_classic() +
  theme(legend.position = c(0.16, 0.18), panel.background = element_rect(colour = "black"), axis.text.x = element_text(angle = 45, vjust = 0.5)) 
...
```{r}
# save plot
ggsave("Past_Host_DEG_barplot.pdf", plot = plot.combo,  width = 5, height = 4, units = "in", dpi = 1200) #width = 6.5, height = 4.5
```
