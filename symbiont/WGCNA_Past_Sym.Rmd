...
# Symbionts from *Porites astreoides*
...
par(cex = 0.6);
par(mar = c(0,4,2,0))
plot(sampleTree, main = "P. astreoides Symbionts - Sample ID", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2)
```

*NO_265, NO_268, NO_280 have high null counts (69-82%) (per DESeq2 analysis)*
...
par(cex = 0.6)
par(mar = c(0,4,2,0))
plot(sampleTree, main = "Porites astreoides Symbionts",  sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2) 
```
...
save file
```{r}
pdf(file = "Past_Sym_dendrogram_SampleID.pdf", width = 13, height = 9)
par(cex = 0.6)
par(mar = c(0,4,2,0))
...
save(dat, traits, file="Past_Sym_SamplesAndTraits.RData")
...
save plot
```{r}
pdf("Past_Sym_dendrogram_Trait-heatmap.pdf") 
...
```
pickSoftThreshold: will use block size 6303.
 pickSoftThreshold: calculating connectivity for given powers...
   ..working on genes 1 through 6303 of 7098
   ..working on genes 6304 through 7098 of 7098
   Power SFT.R.sq  slope truncated.R.sq mean.k. median.k. max.k.
1      2    0.873  2.080          0.837  3240.0   3400.00   4390
2      4    0.166  0.445          0.374  1700.0   1790.00   2980
3      6    0.080 -0.342          0.570   969.0    992.00   2130
4      8    0.312 -0.811          0.781   586.0    576.00   1570
5     10    0.476 -1.160          0.871   371.0    344.00   1190
6     12    0.568 -1.430          0.913   244.0    213.00    923
7     14    0.635 -1.590          0.945   165.0    135.00    726
8     16    0.698 -1.690          0.969   114.0     87.40    580
9     18    0.747 -1.790          0.980    80.9     57.80    468
10    20    0.778 -1.870          0.986    58.4     38.80    382
11    22    0.801 -1.940          0.987    42.8     26.50    315
12    24    0.812 -2.010          0.990    31.9     18.30    261
13    26    0.833 -2.020          0.993    24.0     12.80    218
14    28    0.842 -2.060          0.992    18.4      9.14    184
15    30    0.850 -2.080          0.995    14.2      6.57    156


```{r}
# read in SoftPower object generated in R
sft <- readRDS("Past_Sym_SoftThreshold.rds")
...
```{r}
save(dynamicMods_hybrid, MEList, MEs, MEDiss, METree, file= "Past_Sym_WGCNA_ME_Network.RData")
```
...
save plot
```{r}
pdf(file="Past_Sym_WGCNA_MergeNetwork.pdf", width=20, height=20)
#pdf(file = "___.pdf", wi = 9, he = 6)
...
#MEs_final = mergedMEs
save(mergedMEs, moduleLabels, moduleColors_merged, MEDiss_merged, METree_merged, geneTree, file= "Past_Sym_WGCNA_ME_Network_merged.RData")
```
...
save pdf
```{r}
pdf(file="Past_Sym_WGCNA_Eigengene-dendrogram-heatmap_smaller.pdf", width=8, height=7) #width=10, height=9
plotEigengeneNetworks(mergedMEs, "", marDendro = c(0,4,1,2), marHeatmap = c(3,4,1,2))
dev.off()
```
...
cex.text = 0.7,
zlim = c(-0.7,0.7),
main = paste("Porites astreoides Symbionts Module-Trait correlations"))
```


save plot
```{r}
pdf("Past_Sym_module-trait_heatmap.pdf")
...
cex.text = 0.7,
zlim = c(-0.7,0.7),
main = paste("Porites astreoides symbionts"))
...
skyblue <- names(dat)[moduleColors_merged == "skyblue"]
skyblue <- as.data.frame(skyblue)
names(skyblue)[names(skyblue) == "skyblue"] <- "gene"
skyblue$module <- "skyblue"

steelblue <- names(dat)[moduleColors_merged == "steelblue"]
steelblue <- as.data.frame(steelblue)
names(steelblue)[names(steelblue) == "steelblue"] <- "gene"
steelblue$module <- "steelblue"

darkturquoise <- names(dat)[moduleColors_merged == "darkturquoise"]
darkturquoise <- as.data.frame(darkturquoise)
names(darkturquoise)[names(darkturquoise) == "darkturquoise"] <- "gene"
darkturquoise$module <- "darkturquoise"

darkgreen <- names(dat)[moduleColors_merged == "darkgreen"]
darkgreen <- as.data.frame(darkgreen)
names(darkgreen)[names(darkgreen) == "darkgreen"] <- "gene"
darkgreen$module <- "darkgreen"

saddlebrown <- names(dat)[moduleColors_merged == "saddlebrown"]
saddlebrown <- as.data.frame(saddlebrown)
names(saddlebrown)[names(saddlebrown) == "saddlebrown"] <- "gene"
saddlebrown$module <- "saddlebrown"

darkorange <- names(dat)[moduleColors_merged == "darkorange"]
darkorange <- as.data.frame(darkorange)
names(darkorange)[names(darkorange) == "darkorange"] <- "gene"
...
lightgreen <- names(dat)[moduleColors_merged == "lightgreen"]
lightgreen <- as.data.frame(lightgreen)
names(lightgreen)[names(lightgreen) == "lightgreen"] <- "gene"
lightgreen$module <- "lightgreen"

grey <- names(dat)[moduleColors_merged == "grey"]
grey <- as.data.frame(grey)
names(grey)[names(grey) == "grey"] <- "gene"
grey$module <- "grey"

grey60 <- names(dat)[moduleColors_merged == "grey60"]
grey60 <- as.data.frame(grey60)
names(grey60)[names(grey60) == "grey60"] <- "gene"
grey60$module <- "grey60"

summary <- rbind(cyan, lightcyan, skyblue, steelblue, blue, darkgreen, lightgreen, grey, grey60, darkorange, darkturquoise, saddlebrown)

write.csv(file = "Past_Sym_WGCNA_module_genes.csv", x = summary)
```
