...
Same transcriptome reference, just used different suffixes for counts of the different species
```{r}
...
Consolidate below steps
```{r}
# Split out contigs from each orthogroup
multi.ortho.all <- multi.ortho

# be aware of the blank space " " after the comma after the split function !!
multi.ortho.all <- separate_rows(multi.ortho.all, gene_Sid, sep = ", ")
multi.ortho.all <- separate_rows(multi.ortho.all, isotig, sep = ", ")

multi.ortho.all <- past_seq2iso %>% 
  inner_join(multi.ortho.all, by = "isotig")

head(multi.ortho.all)
```


Previous working out of steps:

Split out contigs from each orthogroup
```{r}
#multi.ortho.Sid <- multi.ortho[ , 1:2]

multi.ortho.Sid <- multi.ortho
...
#multi.ortho.Sid[sapply(multi.ortho.Sid, is.character)] <- lapply(multi.ortho.Sid[sapply(multi.ortho.Sid, is.character)], as.factor)
...
# did not finish re-working
# dim(multi.ortho.Past)
# 
# multi.ortho.all <- multi.ortho.Sid
# 
# multi.ortho.all <- separate_rows(multi.ortho.all, isotig, sep = ", ")
# 
# multi.ortho.all[sapply(multi.ortho.all, is.character)] <- lapply(multi.ortho.all[sapply(multi.ortho.all, is.character)],
#                                        as.factor)
# 
# dim(multi.ortho.all)
```
...
OO_OC_Sid$gene_Sid <- as.factor(OO_OC_Sid$gene)
head(OO_OC_Sid)
```

```{r}
multi.ortho.OO_OC_Past <- inner_join(multi.ortho.all, OO_OC_Past, by = "gene")

multi.ortho.OO_OC_Past$gene_Sid <- as.factor(multi.ortho.OO_OC_Past$gene_Sid)

head(multi.ortho.OO_OC_Past)
```

```{r}
intersect(OO_OC_Sid$gene_Sid, OO_OC_Past$gene_Sid)
```
...
past_1.to.1.isogroups_OO_OC <- past_1.to.1.isogroups_OO_OC[,1:4]

past_1.to.1.isogroups_OO_OC <- past_1.to.1.isogroups_OO_OC %>% 
  dplyr::rename(gene.Past = gene, gene.Sid = gene_Sid)

orthogroups.OO_OC <- full_join(multi.isogroups_OO_OC, past_1.to.1.isogroups_OO_OC, by = c("isotig", "gene.Past", "orthogroup", "gene.Sid"))
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      labs(title = "Ojo-Ojo vs. Ojo-Control (82 orthogroups)") +
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Ojo vs. Ojo-Control (86 orthogroups)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Ojo vs. Lagoon-Ojo (6 orthogroups)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
colnames(Sid.counts.transformed.vst)
counts_de <- Sid.counts.transformed.vst[,c(2,4,6,9,11,13,15,17,19,21,22)]
counts_de <- counts_de[rownames(counts_de) %in% sigChanges,]
dim(counts_de)
```
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Ojo vs. Lagoon-Ojo (7 orthogroups)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
colnames(Past.counts.transformed.vst)
counts_de <- Past.counts.transformed.vst[,c(10:21)]
counts_de <- counts_de[rownames(counts_de) %in% sigChanges,]
dim(counts_de)
```
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Reef-Ojo vs. Reef-Control (180 orthogroups)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
colnames(Sid.counts.transformed.vst)
counts_de <- Sid.counts.transformed.vst[,c(23:37)]
counts_de <- counts_de[rownames(counts_de) %in% sigChanges,]
dim(counts_de)
```
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Reef-Ojo vs. Reef-Control (3 orthogroups)") +
      labs(color = "Origin.Destination", fill = "Origin.Destination") +
      theme_cowplot()
